...
| **RSS set: BBC / Sky / Politico / The Hill** (v3) | public feeds, headline+link display | **Adopt (wired)** | 7 feeds live-probed 200 (Politico politics-news valid-but-empty at probe; congress.xml carries items). Fetched once/run, day-cached, keyword-filtered per market, title-hash deduped. Reuters/AP no public RSS; Google News RSS personal-use-only — skipped. |
| **Wikipedia RSP tiers + Iffy Index** (v3) | CC BY-SA 4.0 / CC BY 4.0, attribution | **Adopted (was proposal)** | RSP row status classes parsed via MediaWiki API (439 entries incl. Sky News UK/AU split); Iffy domain list via their published sheet CSV (2,040 domains). Declared status→weight map in `newsagent/sourceweights.py`; weekly ATTENDED refresh; outage serves stale cache. Uncovered sources (newsletters, WP-CE) still awaiting the sign-off table in [[newsagent_observatory_v3_findings]]. |
| **ING newsletters via read-only inbox** (v3) | private/licensed content | **Adopt (Stage-A input only)** | IMAP read-only (BODY.PEEK); NEVER displayed publicly (generic label only, test-enforced); credential pending from Justin. Analysis-grade text, richer than headlines. |
