...
**Open items for Justin:** (1) Scheme-A sign-off incl. the uncovered-source table (ING/WP-CE/unknown); (2) read-only email credential `secrets/email_imap.json` for newsletter ingestion; (3) `GUARDIAN_API_KEY` (registered) + `ANTHROPIC_API_KEY` + cron-env `GOOGLE_APPLICATION_CREDENTIALS` for the future unattended switch; (4) website handoff stays parked; (5) July settlements (`sf settle`): US–Iran meeting 07-17, Fed 07-29, Hormuz + MOU 07-31.
