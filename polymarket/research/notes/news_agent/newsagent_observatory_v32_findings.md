...
created: 2026-07-05
status: shipped — attended/manual; lean table + declared multipliers APPROVED as shipped 2026-08-24 and band_mult selector call APPROVED (keep 0.5); website parked
owner: justin
project: polymarket
para: project
hubs:
...
> **RESOLVED — Justin, 2026-08-24: BLESSED, band_mult stays 0.5.** The narrowest-knob-at-nominal reading is the official one; the forward-ledger rescale at ≥20 settled forecasts (pre-registered) supersedes it, and at 4 settled it has not triggered. Re-checked at the 2026-08-24 refit under the newly activated Scheme-A weights: the selector again returns **0.5** (bucket coverage 1.0 vs nominal 0.8 on the 302 backfill pairs), so the decision costs nothing today.
...
1. ~~**Lean sign-off (still open):** the table + declared multipliers (center/unrated 1.0 · |1| 0.9 · |2| 0.75).~~ **DONE 2026-08-24 — APPROVED AS SHIPPED.** No code change: the AllSides-seeded table and the declared extremity multipliers stay exactly as they run today, and they remain refit-covered (a veto would still be a one-line revert plus a refit). **The band_mult selector call is also APPROVED: keep 0.5** — the "narrowest knob AT nominal" reading stands, with the pre-registered recheck at the next coverage rescale (≥20 settled; we are at 4).
...
4. **Unchanged from v3/v3.1 — now mostly ANSWERED (2026-08-24):** Scheme-A uncovered-source weights **APPROVED and ACTIVATED** (ING 0.9 / WP-CE 0.8 / unknown 0.5 / Bloomberg 0.9 / bank desks 0.9 — no longer neutral 1.0; **α refit 2.7 → 2.85**, the whole move attributable to this activation); `GUARDIAN_API_KEY` **loading built** (git-ignored `.env` read at import) but its **value still needs pasting**; `GEMINI_API_KEY` **still open**; July settlements **DONE** (4 settled + scored, slate refreshed, refit, first public Brier 0.0794). See [[newsagent_observatory_v33_findings]].
