...
- **EXECUTED 2026-07-21 (operator-scheduled):** Tier 2 items 1–3 shipped as code and Tier 3 ran on a pre-registered 14-day real-L2 window → [[dali_features_real_l2_remeasure_findings]]. Outcome: the class-B passive/maker closures (A14c/A14h/A18/P2) are **confirmed-closed-on-real-data** through the coded queue bracket (upgraded to robust); the A17 feature-signal reading is **resolved: the features DO carry conditional signal** under a doubly-disjoint clean split (OFI-on-politics excepted); the taker floor on the new markets is much lower than dali's crypto floor but still binding. No edge branch reopens; the surviving open question is the quoter-gate-input use of the signal (outcome 1), now the best-motivated next MM experiment.
