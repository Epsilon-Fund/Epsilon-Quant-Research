...
| closed positions | [[polymarket/research/data/closed_positions.parquet|closed_positions.parquet]] | ⚠️ **CONDEMNED 2026-07-21 pending bundle-aware regeneration** ([[pm_prealvaro_pipeline_trust_audit_findings]] Finding 2): aggressor double-count + phantom cross-token legs on `_matchOrders` bundles. Resolved `(address, market, outcome)` positions with synthetic redemption. | [[RESEARCH_FINDINGS]], [[block_e_audit]], trader profiles, copytrade |
| trader panel | [[polymarket/research/data/traders.parquet|traders.parquet]] | ⚠️ **CONDEMNED 2026-07-21** (inherits closed_positions defect): PnL/volume *levels* for taker-heavy wallets and style columns unreliable. Per-wallet metrics, style, bankroll, drawdown, and operator flag. | cohorts, profiles, [[api_reconciliation_v1]], copytrade |
...
| directionality | [[polymarket/research/data/directionality_classification/traders_directionality.parquet|traders_directionality.parquet]] | ⚠️ **CONDEMNED 2026-07-21** (phantom cross-token legs bias `primary_style` toward arb_like/two_sided; [[pm_prealvaro_pipeline_trust_audit_findings]] Finding 2). Directional/arb-like wallet style sidecar — Join-2 market screen may use it as a *preference* only, never a gate. | [[block_e_audit]], [[mm_politics_negrisk_live_loop_design]] |
...
| live CLOB captures | `polymarket/research/data/live_clob/**/*.jsonl` (local copy PRUNED — now R2-only at `r2:epsilon-polymarket-data/research-live-clob/`) | Raw captured public PM CLOB book/trade events for A0/A0b/A0c/dali replay and MM Stage-1 measurement. Public L2 is anonymous; see [[mm_clob_capture_semantics]] before using it for trade-vs-cancel attribution. The continuous capture now runs 24/7 on a VPS → Cloudflare R2; cloud path, shard layout, format, and `rclone` pull commands are in [[mm_vps_capture_setup]]. | [[block_a0_runbook]], [[mm_clob_capture_semantics]], [[mm_vps_capture_setup]], A1/A11/A12/A13/A14+ replay notes |
...
- `data/analysis/csv_outputs/market_maps/ipo_tapes/` — cached mega-IPO day-1 tapes (CBRS spot 1m listing day + daily OHLCV for ARM/RIVN/BABA/META/RDDT, by `scripts/spcx_ipo_unwind_tape_study.py`; CBRS spot 5m post-listing + CBRS hourly HL funding through the conversion, by `scripts/spcx_perp_exit_mechanics.py`). The CBRS 1m/5m windows are PURGED from Yahoo ~30d after listing, so these caches are the only surviving copy — do not delete. Timestamps are raw venue OPEN-time stamps; shift to close-time before cross-series comparison (see the S6 method note). See [[spcx_ipo_unwind_tape_findings]].
