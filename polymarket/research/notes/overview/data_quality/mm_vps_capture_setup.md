...
> **CORRECTIONS (2026-08-25, VPS reboot outage + recovery — [[2026-08-25_l2_pipeline_recovery]]):** Known-failure mode added. A **hard reboot (2026-08-21 21:13:15 UTC)** truncated the three then-open shards. **compress.service crashed on the first bad shard every run for three days** (`process_files()` let one `zlib.error` abort the whole run), so ~261 shards never compressed → no Parquet → no sync → local raw never pruned → disk hit 93%; R2 Parquet froze at 2026-08-21. **Two things about the corruption to remember:** (a) after the reboot capture re-opened each hourly file in append mode, so a truncated file is **two gzip members** — a broken pre-reboot member followed by a *complete* post-reboot member; `zcat`/`gzip.open()` stop at the break and miss ~90% of the data. Recover with **`compression/salvage_truncated_shard.py`** (member-aware). (b) Real loss was only a **~9-minute unflushed-buffer window per universe** (21:04:18→21:13:17), not the reboot downtime (~2 s). **Fix (deployed 2026-08-25):** `process_files()` now **quarantines-and-continues** — a bad shard is logged, appended to `data/parquet/_quarantine.txt` (a **to-salvage list, not a discard list**), skipped, and the run exits non-zero after processing everything else. Note the recovery session **stopped the collector** (capture + all timers `disable --now`, reversible) and left the Parquet backlog (08-21 h21→08-26) **to be rebuilt off-server from R2 raw** per [[DATA_LAYER_PLAN]]. R2 raw now covers 08-21→08-26; R2 Parquet 06-19→08-21.
>
