...
Every pre-Alvaro branch **stays closed**; no reopens. The corpus's negatives are now stress-tested against the institutional bar and survive — mostly a fortiori. What changed is the *record*: two headline numbers corrected (73.7% and the 36% framing), one scope claim falsified ("PnL unaffected"), one sub-claim condemned (A17 calibration), three data artifacts condemned pending regeneration, and two live-path flags installed. Next: Phase 3 (code efficiency pass), then the regeneration work (action 1–2) as its own thread when the operator schedules it.
