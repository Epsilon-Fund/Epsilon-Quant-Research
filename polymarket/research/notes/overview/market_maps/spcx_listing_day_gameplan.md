...
- **Clock re-anchored to the cross** (per §4): Phase A = first 10–15 min post-print; Phase B/C windows defined in minutes-since-cross, not CET wall-clock. Block S2 calibrates the windows on Cerebras + mega-IPO tape (when, in minutes after first print, did the volume-weighted peak and the fade onset actually occur?).
...
- Fade-risk asymmetry at 4× oversubscription: the bigger the book, the more pre-IPO holders/insiders are *not* sellers day 1 (lockups) but also the hotter the open — S2 should check whether mega-IPO fade size correlates with oversubscription before trusting the -13% fade estimate.

### 5.3 Day-of screen map + indicator set (interim — Block S3 verifies feeds, S2 calibrates thresholds)
...
**D0 — Now → Thu morning (Jun 10–11).** No position. Snapshot the calculator when convenient (cached `latest.json`); continuous `--watch --parquet-log` is NOT required before Friday — S1's decay fit uses HL candle history. The logger becomes mandatory **Thursday evening onward** (pricing-night reaction, allocation morning, in-session liq alarm + funding/oracle/OI capture + purge insurance). Human actions: confirm TR day-1 sell capability + order types in-app (Alvaro's research items 1–10; Block S4 does the desk research, but the in-app support answer is human-only and is the single highest-value 15 minutes left this week). Verify TR subscription/withdrawal deadline. Set up TradingView watchlist + alerts ($135 / $157 / $162 / S1 thresholds).

**D1 — Pricing night (Thu ~22:00 CET, 424B).** Read the final price. If $135: basis = perp − 135, proceed. If >$135: recompute everything (S1 grid row). If S1's pre-hedge rule is green (expected-fill-weighted EV of hedging *tonight* > waiting, at a pessimistic fill), open the pre-hedge tranche — sized to the **pessimistic-fill** shares only (never above the 10%-fill row of §3). Otherwise do nothing.

**D2 — Allocation (Fri ~8:00 CET).** Read exact share count. Compute hedge sleeve = min(fill, margin-supported at ≤1.5×) **if** net basis ≥ S1 threshold; top up (or open) the short to h=1 on those shares. Decide residual-sleeve tranche sizes from the §3 row that materialized. Communicate the split to Alvaro (his scenario matrix keys off the same allocation number).
...
**D4 — Display-only period → IPO cross (15:30 → ~17:00–20:00 CET).** Watch NOII indicative price (S3 source) + perp. No selling into the cross or first prints. Note cross time and price; start the S2-calibrated tranche clock.
