...
Engine (`mm_engine/`): **[[mm_engine_build_log]]** (the recap — chronological build log + component map + where-we-are; start here) · [[2026-06-23_mm_engine_phase01_buildplan]] (Phase 0/1 build plan + Alvaro handoff) · [[mm_engine_fee_pnl_settlement]] (fee/rebate, three-way PnL, settlement, record→replay) · [[mm_engine_queue_models]] (the three queue models — Optimistic/RiskAverse/Prob — and the fill-rate bound they produce) · [[mm_queue_model_audit_fixes_findings]] (forget() teardown, logical floor, order-invariant netting + L2 coincidence measurement) · [[mm_join1_reconciliation_findings]] (**JOIN 1 reconciliation — PASS**: 0% gap per model, book ~99% faithful once an intra-ms ordering artifact is accounted for, model bracket + queue_ahead≤depth on real VPS Parquet, byte-identical determinism; surfaced + fixed a `replay_parquet` live-schema-drift bug) · [[mm_reconstruction_audit_findings]] (book-reconstruction accuracy, JOIN-1 corrected) · [[mm_engine_join2a_bridge_findings]] (**JOIN 2a live bridge — DRY-RUN/mock**: the backtest's SAME strategy/queue/book code placing orders through the existing execution/maker signer + safety harness; only the fill path swaps; runs in the execution venv importing mm_engine as a path dependency; telemetry a superset of the backtest; 22 new tests + 281 execution suite green, mm_engine 85 untouched; a 10-agent adversarial review caught+fixed 3 live-path bugs) · [[mm_symmetric_quoter_validation_findings]] (**Task 4 validation / eval gates** — per-market viable/fragile/dead on the ~11-day R2 clone, bracketed by queue + CIs: politics 8/12 VIABLE / esports 5/12 (bimodal); breakeven = spread-vs-adverse-selection since captured fee=0; naive quoter is a directional-inventory bet → inventory control is Task 5's gate; DSR/CPCV wired-but-dormant on the 0-param quoter) · [[mm_market_screen_and_ttr_regime_findings]] (**Task-5 design inputs** — the failure-driver market screen [certified real-time signals: large aggressor + book imbalance at fill; pre-quote gate: avoid ~50¢-parked markets, prefer calm low-mid-vol books; **half-spread refuted as a safety selector**] + the **NegRisk time-to-resolution regime**: near-expiry politics is toxic while mid-life is favorable, but the durable outrights are observed **mid-life only** so their near-expiry toxicity is a **live-only risk** → pull quotes near expiry as a live-calibrated rule) · [[mm_task5_inventory_quoter_findings]] (**Task 5 — inventory-managed quoter, gated ladder, COSTED**: once exits are charged the symmetric baseline *loses* OOS in both categories (politics −0.96¢/ct, esports −36¢/ct — the inventory bet exposed); the v1 `InventoryAwareQuoter` [microprice skew + tight cap + politics τ-flatten + velocity gate] is the **only OOS-surviving rung** — beats the baseline on the pre-registered event-group gate in both categories and ends books flat — while every A-S rung and the basket-carry alternative DROP (several certified worse); but v1's own OOS level is +0.03¢ politics (CI through zero) / −1.2¢ esports, PBO ≈ 0.5, DSR fails → **damage control proven, no deployable edge claimed**; disposition = Join-2 1-contract live measurement loop with v1-politics as the config) · [[mm_engine_join2_machinery_findings]] (**JOIN 2b/2c/2d machinery — DRY-RUN/mock**: the latency probe harness (`--mode mm_latency`, unexecutable 0.001/0.999 probes gated exactly like quotes), the live-run wiring (5-screen market selection script, measured-latency hand-off, hard contracts inventory cap, reconcile cadence), and the calibration pipeline (`--mode mm_calibrate`, external `ProbQueue.f` grid fit + bracket-collapse report, exact ground-truth recovery on synthetic fills); no-real-order conjunction proven by test; operator procedure in [[MM_JOIN2_RUNBOOK]]; the real 1-contract run is the operator's next step).
Dali-lineage re-measure on real L2: [[dali_features_real_l2_remeasure_findings]] (**2026-07-21** — Tier-2 workflow hardening shipped [one metric module + required capture-quality gate + book-measured costs]; dali passive/maker closures confirmed-closed-on-real-data through the coded queue bracket; A17's feature-signal reading resolved positive under a clean split → TOB/microprice state is the best-motivated NSQ gate-input candidate; new-market taker floor measured, fee=0 documentary).
