...
- **Takeaway:** run all three; they fail in different ways and a strategy needs to survive all of them. The pre-registered gate is **deflated Sharpe > 0 AND PBO < 0.5 AND post-haircut lower-CI Sharpe > 0.25**. Statistical survival is necessary but not sufficient — a survivor can still be too small to trade ([[CODEX]] § Realism calibration).
...
## Monte-Carlo extensions (complement, not replacement)

The harness already does one Monte Carlo — the **stationary block bootstrap** behind every Sharpe CI. Two further MC flavours are sometimes proposed; only one is genuinely additive for *overfitting*:

- **Synthetic-null / regime-aware MC (worth adding).** Re-run the *entire search pipeline* on no-edge data (block-bootstrap or regime-stratify the underlying returns so serial signal is destroyed but vol/marginals survive), and ask how often the search manufactures a Sharpe as high as the real one. This is the **empirical version of the DSR haircut** — it subjects the *whole search* to the null, not just the winner, and captures fat tails / autocorrelation / regime structure that DSR's analytic formula assumes away. It is **deliverable 6 of the original spec** ([[2026-06-05_novelty_frontier_map]] Prompt 1), deferred from the first build. For a result that already clears DSR + Reality Check it tends to confirm rather than overturn, but it is the right next piece of reusable infra and the natural home for the existing HMM regime labels. Gate: real Sharpe > 95th percentile of the synthetic-null distribution.
- **Trade-shuffle / drawdown MC (skip for overfitting).** Permuting trade order to build a max-drawdown distribution is a *position-sizing / path-risk* tool, not an overfitting test — useful for sizing a long-only book against sequencing risk, but it answers a different question and is not part of this gate.

**Bottom line:** DSR + PBO + Reality Check + bootstrap CIs cover the *selection-bias* question thoroughly; the synthetic-null MC is the one worthwhile complement, and a deferred-but-pre-registered one. The trade-shuffle MC belongs in risk tooling, not here.
