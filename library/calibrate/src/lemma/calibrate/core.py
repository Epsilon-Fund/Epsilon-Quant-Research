"""
core.py — calibration scoring engine
====================================
Score how good probabilistic forecasts are. Pure scoring/diagnostics: it makes
NO judgments, and its optional forecast-ledger reader is strictly **read-only**
— the ledger's own append-only state machine (whatever tool maintains it)
remains the single source of truth.

Conventions:
  - ``calibration_table`` uses equal-width ``pd.cut`` binning with columns
    ``prob_bucket / mean_pred_prob / actual_freq / n`` rounded to 3 dp — kept
    byte-compatible with the originating research stack's metric table so
    existing research code can swap one for the other with no regression.
  - Recalibration mirrors the common walk-forward convention
    ``IsotonicRegression(out_of_bounds='clip')`` fit on a held-out set when
    sklearn is available; a pure-numpy PAV fallback runs where it is absent.
...
----------
  ...
  Plot / ledger
    ...
    resolve_ledger_dir(book=None, books=None, ledger_dir=None)      -> Path
    load_scored_forecasts(book=None, books=None, ledger_dir=None)   -> pd.DataFrame
    score_ledger(book=None, n_bins=10, books=None, ledger_dir=None) -> dict
"""
...
def calibration_table(prob, label, n_bins: int = 10) -> pd.DataFrame:
    ...
    Equal-width ``pd.cut`` binning, ``observed=True`` grouping, 4 columns
    rounded to 3 dp — the layout is kept byte-compatible with the originating
    research stack's metric table so existing code can swap one for the other
    with no regression. A well-calibrated forecaster has ``mean_pred_prob ≈
...
def isotonic_recalibrate(p_tr, y_tr, p_apply=None, backend: str = "auto"):
    ...
    a held-out set when sklearn is available; otherwise a pure-numpy PAV fit
    with clip-extrapolation.
...
# ── ledger reader (read-only consumer of an append-only forecast ledger) ────────
#
# Ledger format: a directory containing ``forecasts/events.jsonl`` — one JSON
# object per line; rows with ``type == "scored"`` carry ``id, timestamp,
# final_probability, outcome, brier``. This matches the superforecasting-skill
# ledger convention, but any tool emitting that shape works.


def resolve_ledger_dir(book: str | None = None, *,
                       books: dict | None = None,
                       ledger_dir=None) -> Path:
    """
    Resolve the forecast-ledger directory. Precedence:

      1. ``ledger_dir`` argument (explicit path)
      2. ``$SF_LEDGER_DIR`` environment variable
      3. ``book`` (argument, else ``$SF_BOOK``) looked up in the caller-supplied
         ``books`` mapping (book name -> ledger directory)

    Separate books share no state, so this never guesses: with nothing
    resolvable it raises with a clear remedy.
    """
    if ledger_dir:
    ...
    if books and b in books:
        return Path(books[b]).expanduser()
    raise SystemExit(
        "calibrate: no ledger selected. Pass ledger_dir=<path> (or set "
        "SF_LEDGER_DIR=<path>), or pass book=<name> with a books mapping "
        "(or set SF_BOOK with one). Books share no state, so this refuses "
        "to guess."
    )
...
def load_scored_forecasts(book: str | None = None, *,
    ...
    """
    ledger = resolve_ledger_dir(book, books=books, ledger_dir=ledger_dir)
...
def score_ledger(book: str | None = None, n_bins: int = 10, *,
                 books: dict | None = None, ledger_dir=None) -> dict:
    """
    One-call scorecard over a ledger's scored forecasts: Brier (+ Murphy
    ...
    """
    df = load_scored_forecasts(book, books=books, ledger_dir=ledger_dir)
    if df.empty:
        raise SystemExit(
            "calibrate: no SCORED forecasts in the ledger yet. Settle/score "
            "some forecasts with your ledger tool first."
        )
