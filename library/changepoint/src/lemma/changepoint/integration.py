"""
integration.py — wire the causal break signal into the three common consumers.

1. purged-CV embargo    : turn detected break timestamps into bar indices to
                          embargo around each break, to union into the purge set
                          of a combinatorial purged cross-validation splitter.
2. regime features      : a causal `change_prob` / `run_length` feature column to
                          add to a regime-classification model — "a structural
                          break just fired" is a strong, lookahead-free predictor
...
None of these import any specific classifier or CV engine — they return plain
...
def embargo_indices_from_breaks(index, break_timestamps, embargo_bars: int = 5,
    ...
    positions to your purged-CV splitter: a label spanning a structural break
    leaks regime information across the train/test boundary, so those bars
    should be dropped from training.
    """
...
def changepoint_features(values, *, timestamps=None, name: str = "bocpd",
    ...
    """Causal feature columns for a regime-classification model.
    ...
    from data <= t, safe to merge into a feature matrix that predicts the NEXT
    bar's regime. Lag them by 1 bar in the model if you want strict
    information-at-close semantics.
    """
