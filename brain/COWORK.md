...
**dali — research lineage / redesign trail.** Direct local microstructure-continuation branches were falsified in specific retests, but dali itself is not closed; it is the broader Polymarket research line that kept redesigning into Block K, MM, OD, and future Justin-directed Polymarket work. Key falsification/redesign anchors: [[block_p3prime_oos_findings]], [[block_a0c_holdout_retest_findings]]. **Canon audit (2026-07-19 ledger [[pm_prealvaro_canon_audit_findings]] + 2026-07-21 pipeline trust audit [[pm_prealvaro_pipeline_trust_audit_findings]])** — all closures robust and stress-tested (replay integrity, fill-model bias, fee regime, split design); no reopens. Corrected record: the "73.7% → 36.0% OOS collapse" was a metric-mismatch artifact (descriptive TOB signal replicated ~63% conditional OOS, still non-monetizable); A17's calibration table is condemned as evidence; the historical position pipeline (closed_positions → traders → directionality) is condemned pending bundle-aware regeneration.
- Signal-as-input framing: [[block_a1x_external_note_reconciliation]]
- Capture + replay status: [[dali_capture_status_rollup]] (one-page rollup) → terminal records [[block_a0_capture_status_final]], [[block_a0b_capture_status_final]], [[block_a0c_capture_status_final]], [[block_a0c_crypto_roll_status_final]]; intermediate checkpoints + duplicate audits archived in `notes/dali/archive/`
...
**copytrade (primary thread).** [[copytrade_relayer_implications]], [[copytrade_attribution_repartition_findings]], [[relayer_dig_findings]], [[block_b_reinterpretation]], [[block_e_lite_findings]], [[block_e_audit]], [[profile_domah]], [[phase5_design]], [[copytrade_spread_surface_mtm_findings]] (SPREAD-2: the SPREAD-1b-validated trade-time surface replaces the flat-3c slippage fallback across all three copy evaluators + lookahead-free MTM equity per cohort; no deployment verdict flips, flat-3c shown to under-charge HFT makers / over-charge tight-book leaders; §4b Domah MTM: identical fills at his own prices +$1.14M Sharpe 0.95 vs pure-taker copy −$429k Sharpe −0.36 with a −$880k trough — the copy loss is an execution-price problem visible as a never-recovering equity path), [[weather_ftc_state]], [[polymarket/archive/prediction_markets_falcon_pipeline/README|prediction-markets pipeline (archived — Falcon vendor dead end)]]. **Canon audit (2026-07-19): all copytrade notes carry verdict banners — ledger [[pm_prealvaro_canon_audit_findings]].**
- Execution + probes: [[polymarket/midas/README|Midas README]], [[polymarket/execution/README|Polymarket execution README]], [[polymarket/execution/PLAN|execution plan]], [[polymarket/execution/CLAUDE|execution rules]], [[polymarket/execution/scripts/SMOKE_REAL|SMOKE_REAL]], [[polymarket/execution/tests/probes/WS_PROBE_FINDINGS|WS probe findings]], [[polymarket/execution/tests/probes/NEGRISK_FINDINGS|NegRisk findings]], [[polymarket/archive/midas_audit|Midas audit]].
...
SpaceX IPO cross-market handoff: [[spacex_ipo_market_map_handoff]] maps PM, Hyperliquid, TradingView, proxy funds, and the agent build plan for the SpaceX overvaluation/EV thread. Companion coworker angle: [[spacex_ipo_coworker_addendum]] covers the new DOCX/PNG material on Class A vs Class B, `SPCX` vs `SPACEX`, Trade Republic, TradingView, and PCHIP distribution diagnostics. Listing-day execution: [[spcx_listing_day_gameplan]] — the two-sleeve decision tree (convergence lock + sell-the-pop), IPO-cross mechanics, unwind rules, and research blocks S1–S4.
