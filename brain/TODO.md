...
- [ ] **S5 — Live PM-PDF + cross-venue implied-price monitor**: terminal-first (optional static `--html`, no server/Streamlit) PCHIP survivor fit on the PM 16-strike surface, polled 30–60s, with per-share stats, bucket-vs-continuous mispricing, perp/PM gap, parquet poll log. Serves the PEAK trigger + Alvaro's PM leg (his items 14–15). The convergence web dashboard stays declined; this is a different, decision-attached object.
