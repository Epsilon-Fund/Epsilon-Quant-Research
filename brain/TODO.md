...
> Last updated: 2026-06-10 (SPCX listing-day gameplan written — [[spcx_listing_day_gameplan]]: two-sleeve decision tree (convergence lock + sell-the-pop) reconciled with Alvaro's playbook; book now ~4× oversubscribed, perp ~$157/+16% and fading; research blocks S1–S4 queued for Claude Code before Friday).
...
- [ ] **S1 — Hedge grid + pre-hedge timing rule** (supersedes the fill-price × fill-size enhancement): fill-price $135→$162 × fill {10/25/50/100%} × €2k-margin constraint at 1×/1.5×; basis-decay fit on watch-log + HL candles; pre-hedge-at-{now/pricing/allocation} EV → one pre-registered rule ("hedge X shares at node Y iff basis ≥ Z"). Highest priority. Decay fit uses HL hourly/15m candle history (sufficient); `--watch --parquet-log` only becomes mandatory Thursday evening → Friday (liq alarm, funding/oracle capture, purge insurance).
- [ ] **S2 — Unwind schedule from mega-IPO tape**: minutes-from-cross to peak/fade across CBRS/ARM/RIVN/BABA/FB/RDDT day-1 tapes; 40/40/20 vs TWAP-from-cross vs sell-at-cross net of slippage; fade vs oversubscription.
- [ ] **S3 — IPO-cross visibility + data-source map**: where EU retail watches NOII/indicative price, real-time SPCX, L2; verified TradingView setup (anchored VWAP from first print).
- [ ] **S4 — Trade Republic execution mechanics**: Alvaro's research items 1–10/16–17 from published docs + the exact in-app support question list (human asks at D0).
