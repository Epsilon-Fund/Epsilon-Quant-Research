...
| Reflection pass | Automated by scheduled task `reflection-weekly` (Mondays 09:31, after the hygiene scan) | Signal gathering via read-only subagents, scoring, backlog updates in `brain/reflection/candidates.md`, S-cost accepted builds, monthly repo radar | M/L builds, rubric disputes, and anything human-gated (IP scrub, TCC grants) |
| Skills catalog | Regenerated by the reflection pass (or on demand: `python3 tools/skills_catalog.py`) | `brain/generated/skills_catalog.json` + `skills_dashboard.html` + the committed public-candidate feed `library/catalog.json` | Publishing anything (blocked on the human IP/strategy scrub) |
| WIP exposure audit | Run on demand or by the reflection pass: `python3 tools/wip_audit.py` (add `--dry-run` for summary only) | Read-only report `brain/generated/wip_audit.md` — uncommitted paths by age/thread, never-versioned files, unstaged rename tails (flagging a git-ignored destination), untracked `status: active` handoffs, and commits on no remote per branch | **Every remedy**: staging, committing, pushing, and branch pruning stay human decisions per [[MERGE_PROTOCOL]] — the tool never mutates git |
...
- **Matcher:** keyword/description scoring (always on, deterministic, offline) blended with **in-process** semantic similarity — no daemon. The semantic backend is chosen at runtime, first available wins: `fastembed` (in-process ONNX sentence embeddings — true synonym matching; downloads its small model once, then fully offline) → `tfidf` (scikit-learn TF-IDF cosine) → `bm25` (rank-bm25). Each is a guarded optional import; if none are present Sherpa is keyword-only and says which backend is active. Nothing leaves the machine (Ollama is **no longer required** — this replaced the old gbrain/Ollama call). Combined score = `0.6·keyword + 0.4·semantic` when a backend is active, else keyword alone. `fastembed` vectors are cached under `brain/generated/sherpa_embeddings.json` keyed by description hash (re-embedded only when a description changes). Ties break on skill name (deterministic). Override order with `SHERPA_SEMANTIC_BACKENDS` / `--backends`.
