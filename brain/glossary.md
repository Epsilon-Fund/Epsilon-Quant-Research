...
- **Dali** — historically named as Epsilon's short-horizon OFI/microstructure ML project for Polymarket, but in practice the folder (`polymarket/research/notes/dali/`) holds the **entire pre-MM-era Polymarket research lineage** (Blocks A0–A18, P, I, TFI, sign-convention) that redesigned into Block K → MM/OD. Separate from the copytrade bot. Canon status of every dali note: [[pm_prealvaro_canon_audit_findings]] (2026-07-19 audit — all closures robust, 0 demotions).
