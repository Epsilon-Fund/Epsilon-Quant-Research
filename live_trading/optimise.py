...
LIVE_PARAMS_PATH = os.path.join(os.path.dirname(__file__), 'live_params.json')
...
#
#  Copied verbatim from infrastructure/walkforward/wf_engine.py (_default_score,
#  _default_reject, _run_backtest) so that optimise.py has no runtime dependency
#  on wf_engine. Keep in sync if the research notebooks change these functions.
# ══════════════════════════════════════════════════════════════════════════════
...
def optimise_asset(asset: dict, get_binance_client, get_data, backtest_fn,
    ...
    This is a pre-live fit, not a validation run. Walk-forward OOS validation
    was completed in the research notebooks. Here we use all available data to
    find the best free params given the anchored fixed_params.

    Lookback used: asset["lookback"] (2150 bars by default) — intentionally
    larger than WF_CONFIG["train_bars"] (1050). In walk-forward, train_bars
    had to leave room for OOS test windows; there is no such constraint here.
    More history → better param estimates for the final fit.

    # IS metrics here will look better than walk-forward OOS metrics.
    # That is expected — this is a final fit, not a validation run.
    # OOS validation was done in the research notebooks.
    """
    symbol       = asset["symbol"]
    ...
    # ── Drop indicator warmup bars so the first signal is fully valid ─────────
    ...
    # ── Extract best params and merge with fixed_params ───────────────────────
    # fixed_params always wins — they are anchored from the stability analysis
    best_free   = study.best_params
    best_params = {**best_free, **fixed_params}

    # ── Final IS backtest with best params ────────────────────────────────────
...
def _print_verification_table():
    ...
    for asset in ASSET_CONFIG:
        ...
        # show up to 3 fixed params as key=value
...
def main():
    ...
    import optuna
    import sys as _sys
    _sys.path.insert(0, os.path.join(ROOT, 'infrastructure', 'backtester'))
