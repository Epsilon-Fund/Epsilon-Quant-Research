"""
Epsilon Fund — Trade Journal
Two strategy tabs (Momentum Swing / Momentum No Vol) plus a flat Fund log.
Each strategy tab has per-coin sub-tabs, a search bar, an entry-type filter,
and collapsible statistics expanders.

Closed trades are shown in two tables:
  Table 1 — Trade performance (pairs):    Coin | Entry/Exit dates | Days | Size |
                                           Return % | P&L | MAE | Entry type | Exit reason
  Table 2 — Execution detail (per leg):   Coin | Date | Action | Actual price |
                                           Strat close | Slip % | Slip $ |
                                           Leverage | Theo Lev | Note
"""
...
for _p in (_DASHBOARD_DIR, _LT_DIR): sys.path.insert(0, _p)
...
def _load_pairs():
    return build_trade_pairs(DATA_DIR)


@st.cache_data(ttl=60, show_spinner=False)
def _load_trades(DATA_DIR):
    return load_trades(DATA_DIR)


all_trades = _load_trades(DATA_DIR)
...
def _compute_stats(pairs: list) -> dict:
    ...
    exit_reasons = [p.get('exit_reason', '') for p in pairs]
    n_strat_exit = sum(1 for r in exit_reasons if _normalize_exit_reason(r) == 'Strategy')
    n_disc_exit  = sum(1 for r in exit_reasons if _normalize_exit_reason(r) == 'Discretionary')
    ...
    return {
        ...
        'n_disc':           n_disc,
        'n_strat_exit':     n_strat_exit,
        'n_disc_exit':      n_disc_exit,
    }
...
def _compute_exec_stats(pairs: list) -> dict:
    ...
    return {
        ...
        'best':            best,
        'worst':           worst,
        'avg_pnl_strat':   avg_pnl_strat,
        'avg_pnl_disc':    avg_pnl_disc,
    }
...
def _render_stats_html(s: dict, n_total: int) -> str:
    ...
    streak_str = (f"<span class='{streak_cls}'>"
                  f"{s['cur_streak']} {'win' if s['cur_result']=='W' else 'loss'}"
                  f"{'s' if s['cur_streak'] > 1 else ''}"
                  f"</span>")
...
def _render_exec_html(e: dict, pairs: list) -> str:
    ...
    b = e['best'];  w = e['worst']
    best_str  = f"{b['position_id']}, {b['entry_slippage_pct']:+.2f}%" if b else "—"
    worst_str = f"{w['position_id']}, {w['entry_slippage_pct']:+.2f}%" if w else "—"
    strat_str = _usd(e['avg_pnl_strat']) if e['avg_pnl_strat'] is not None else "—"
    disc_str  = _usd(e['avg_pnl_disc'])  if e['avg_pnl_disc']  is not None else "—"
    ...
    rows = "".join([
        ...
        _stat_row("Best execution trade",        best_str),
        _stat_row("Worst execution trade",       worst_str),
        _stat_row("Avg P&amp;L — strategy exit", strat_str),
        _stat_row("Avg P&amp;L — discretionary", disc_str),
    ])
    return f'<table class="stats-table">{rows}</table>'
...
# ── Exit reason normaliser (handles legacy "Strategy exit" / "Discretionary exit") ──
...
def _render_strategy_tab(strategy_keys,
    ...
    """
    Render a full strategy tab with:
      • coin sub-tabs
      • open positions table
      • Table 1 — trade performance (closed pairs)
      • Table 2 — execution detail (one row per entry/exit leg)
      • strategy & execution statistics expanders

    All numeric cells display at 2 dp.
    """
    ...
    # ── Coin sub-tabs (derived from traded symbols, not config) ──────────────
    ...
    for ct_idx, coin_label in enumerate(coin_tab_labels):
        with coin_tabs[ct_idx]:
            ...
            if tab_open:
                st.caption(
                    f"{len(tab_open)} open position{'s' if len(tab_open) != 1 else ''}"
                )
                open_rows = []
                for t in tab_open:
                    ...
                    etype      = (
                        t.get('entry_type')
                        or derive_entry_reason(t.get('signal_snapshot'),
                                               t.get('discretion_note'))
                    )
                    open_size = (
                        t.get('size_usd')
                        or (t.get('coin_capital') or get_coin_capital(sym, DATA_DIR))
                           * t['actual_leverage']
                    )
                    row = {
                        'Coin':        sym,
            ...
            if tab_closed:
                ...
                st.caption(
                    f"{len(tab_closed)} closed trade{'s' if len(tab_closed) != 1 else ''}"
                )
...
# ══════════════════════════════════════════════════════════════════════════════
#  FUND TAB  —  flat log across all strategies
# ══════════════════════════════════════════════════════════════════════════════
...
with tab_fund:
    ...
    if not fund_rows:
        st.markdown(
            '<p class="no-data-msg">No closed trades match current filters.</p>',
            unsafe_allow_html=True,
        )
    else:
...
# ══════════════════════════════════════════════════════════════════════════════
#  MOMENTUM TAB  (Swing + No Vol combined)
# ══════════════════════════════════════════════════════════════════════════════
