...
<!--
Vendored from https://github.com/deusyu/superforecasting-skill (MIT)
at commit 8913b0847ab63b7aefa0599bdffe00362ee360f2 (2026-06-24).

Local fork (epsilon-quant-research): scripts/sf.py LEDGER_DIR no longer points at
~/.superforecast (a single global log). It is now REPO-CONTROLLED and there is ONE
LEDGER PER BOOK, because the polymarket and crypto projects share no state
(brain/CODEX.md: never cross-import between the two projects):

  SF_BOOK=polymarket  -> polymarket/research/data/superforecast/   (under the
                         already git-ignored polymarket/research/data/)
  SF_BOOK=crypto      -> live_trading/data/superforecast/          (git-ignored)
  SF_LEDGER_DIR=<path> overrides both; with neither set, sf errors rather than
                       guess. The fork adds one `os.environ.get` read for the
                       ledger path ONLY — no credentials, no network, no shell.

The ledger is append-only runtime data; it is git-ignored like trades.json and is
NOT committed. Ledger-path mentions in this file, AGENTS.md, and the event schema
were updated to match the fork. The TRANSITIONS state machine, Brier-at-settle, and
the append-only event write are unchanged — those are the point of the engine.

Provenance/lock: skills-lock.json ("superforecasting"). See SKILL_MAP § Runtime
efficiency skills and brain/handoffs/2026-06-29_superforecasting_skill_vendored.md.
-->
...
A deterministic engine lives at `{baseDir}/scripts/sf.py`. Use it for all persistence, validation, and scoring — never simulate ledger operations in prose. The ledger is **repo-controlled and per-book** (epsilon-quant-research fork): export `SF_BOOK=polymarket` or `SF_BOOK=crypto` before calling `sf` so it writes to that book's ledger (`polymarket/research/data/superforecast/` or `live_trading/data/superforecast/`). The two books share no state; with no `SF_BOOK`/`SF_LEDGER_DIR` set, `sf` errors instead of guessing. Within a book the ledger is shared across runtimes (Codex + Claude Code on the same checkout).
...
# → <book ledger>/forecasts/rendered/<id>.md  (book ledger set by $SF_BOOK)
```
...
# → <book ledger>/reports/calibration_YYYYMMDD_NNN.md (NNN auto-increments per day)
```
