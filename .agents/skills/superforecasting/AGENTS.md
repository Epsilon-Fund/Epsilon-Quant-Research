...
> **epsilon-quant-research fork:** the ledger is repo-controlled and per-book — there is no `~/.superforecast`. Point the engine at a throwaway ledger with `SF_LEDGER_DIR` (or pick a book with `SF_BOOK=polymarket|crypto`) so smoke tests don't pollute a real book. The commands below `export SF_LEDGER_DIR` to a temp dir.

Use a throwaway forecast against an isolated ledger to verify the full pipeline:

```bash
export SF_LEDGER_DIR="$(mktemp -d)/sf-ledger"   # isolated; or `export SF_BOOK=polymarket`
SF=scripts/sf.py
...
Verify: state machine accepts the legal sequence, rejects illegal transitions, Brier computed correctly (`(0.65 - 1)² = 0.1225`), markdown card renders to `$SF_LEDGER_DIR/forecasts/rendered/<id>.md`, calibration report written to `$SF_LEDGER_DIR/reports/calibration_YYYYMMDD_NNN.md` (NNN auto-increments per day).

To clean test data afterward, just remove the throwaway ledger dir (`rm -rf "$SF_LEDGER_DIR"`), or reset its files in place:

```bash
> "$SF_LEDGER_DIR/forecasts/events.jsonl"
echo '{}' > "$SF_LEDGER_DIR/forecasts/active.json"
rm -f "$SF_LEDGER_DIR/forecasts/rendered/"*.md
rm -f "$SF_LEDGER_DIR/reports/"*.md
```
