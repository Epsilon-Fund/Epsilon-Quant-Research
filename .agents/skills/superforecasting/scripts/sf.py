...
Ledger location is repo-controlled, one ledger per book (epsilon-quant-research
local fork): set SF_BOOK=polymarket|crypto (or SF_LEDGER_DIR=<path>). The two
books share no state. See this skill's SKILL.md provenance header for the fork.
Zero external dependencies (Python 3.10+ standard library only; the fork reads
os.environ ONLY to pick the ledger directory — never credentials).
"""
...
# --- Ledger location (epsilon-quant-research local fork) --------------------
# Upstream hard-codes LEDGER_DIR = Path.home() / ".superforecast": a single
# global log pooled across every project. This repo forks it to a REPO-CONTROLLED
# path, ONE LEDGER PER BOOK, because the polymarket and crypto projects share no
# state (brain/CODEX.md: never cross-import between the two projects). A
# repo-relative path keeps both agents (Codex + Claude Code) on the same checkout
# pointed at the same per-book ledger, while keeping the two books separate.
#
# Resolution order:
#   1. $SF_LEDGER_DIR                     explicit path override (wins if set)
#   2. $SF_BOOK in {polymarket, crypto}   repo-relative per-book ledger
#   3. (neither set)                      hard error — refuse to guess; never pool books
#
# os.environ is read ONLY to choose the ledger directory; no credentials are
# read. This is the single intentional deviation from the upstream stdlib-only
# surface (still no network, no shell-out, no secrets).
_REPO_ROOT = Path(__file__).resolve().parents[4]
_BOOK_LEDGERS = {
    "polymarket": _REPO_ROOT / "polymarket" / "research" / "data" / "superforecast",
    "crypto": _REPO_ROOT / "live_trading" / "data" / "superforecast",
}
...
def _resolve_ledger_dir() -> Path:
    ...
    book = os.environ.get("SF_BOOK", "").strip().lower()
    if book in _BOOK_LEDGERS:
        return _BOOK_LEDGERS[book]
    raise SystemExit(
        "sf: error: no ledger selected. Set SF_BOOK=polymarket|crypto "
        "(or SF_LEDGER_DIR=<path>) before running sf. The two books share no "
        "state, so sf refuses to guess which ledger to write."
    )
