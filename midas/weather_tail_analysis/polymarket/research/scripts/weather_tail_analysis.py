...
REPO_ROOT = THIS.parents[3]                # Epsilon-Quant-Research

DATA_OUT = RESEARCH_ROOT / "data" / "analysis"
DATA_OUT.mkdir(parents=True, exist_ok=True)

MARKETS_PARQUET = REPO_ROOT / "polymarket-copy" / "data" / "markets" / "markets_2026-05-06.parquet"
TRADES_GLOB     = str(REPO_ROOT / "polymarket-copy" / "data" / "trades" / "*.parquet")
CLOSED_POS      = REPO_ROOT / "polymarket-copy" / "data" / "closed_positions.parquet"
...
def compute_crossings(con: duckdb.DuckDBPyConnection, window_hours: int) -> pd.DataFrame:
    ...
    barrier_exprs = ",\n            ".join(
        f"MIN(CASE WHEN price >= {p} THEN ts END) AS first_cross_{int(p*100):03d}"
        for p in BARRIERS
    )
    q = f"""
    WITH wm AS (
